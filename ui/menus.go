...
func saveFileDialog(app *AppInit) {
	dialog.ShowFileSave(func(uc fyne.URIWriteCloser, e error) {
		if uc == nil {
			return
		} else {
			err := png.Encode(uc, app.PixlCanvas.PixelData)
			if err != nil {
				dialog.ShowError(err, app.PixlWindow)
				return
			}

			app.State.SetFilePath(uc.URI().Path())
		}
	}, app.PixlWindow)
}
...
func BuildSaveMenu(app *AppInit) *fyne.MenuItem {
	return fyne.NewMenuItem("Save", func() {
		if app.State.FilePath == "" {
			...
			e := png.Encode(fh, app.PixlCanvas.PixelData)
			if e != nil {
...
func BuildNewMenu(app *AppInit) *fyne.MenuItem {
	return fyne.NewMenuItem("New image", func() {
